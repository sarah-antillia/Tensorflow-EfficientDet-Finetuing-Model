...
from EfficientDetFinetuningModel import EfficientDetFinetuningModel

#from DetectConfigParser       import DetectConfigParser

# This is take from google/autom/efficientdet/model_inspector

class EfficientDetModelInspector(EfficientDetFinetuningModel):
  """A simple helper class for inspecting a model."""

  def __init__(self, config_file):   #, detect_file_or_dir):
    ...
    self.labels_shape = [self.training_losses_filebatch_size, model_config.num_classes]
    ...
    if model_config.data_format == 'channels_first':
      self.inputs_shape = [self.train_batch_size, 3, height, width]
    else:
      self.inputs_shape = [self.train_batch_size, height, width, 3]

    self.model_config = model_config
    print("--- runmode {}".format(self.train_mode))
  ...
  def saved_model_benchmark(self,
    ...
    raw_images = []
    all_files = list(tf.io.gfile.glob(image_path_pattern))
    if len(all_files) < self.train_batch_size:
      all_files = all_files * (self.train_batch_size // len(all_files) + 1)
    raw_images = [np.array(Image.open(f)) for f in all_files[:self.train_batch_size]]
  ...
  def inference_single_image(self, image_image_path, output_dir, **kwargs):
    driver = inference.InferenceDriver(self.model_name, self.model_dir,
  ...
  def build_and_save_model(self):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
      ...
      if self.model_dir:
        ...
        inference.restore_ckpt(sess, self.model_dir,
  ...
  def run_model(self, runmode=""):
    ...
    if self.runmode == 'dry':
      ...
      if self.runmode == 'saved_model':
        ...
        self.inference_single_image(self.input_images,
                                    self.output_image_dir, **config_dict)
      elif self.runmode == 'saved_model_infer':
        self.saved_model_inference(self.input_images,
                                   self.output_images_dir, **config_dict)
...
def main(_):
  ...
  if len(sys.argv)==2:
    ...
    raise Exception("Usage: python EfficientDetModelInspector.py config_file")
  
  if os.path.exists(config_file) == False:
    raise Exception("Not found train_config {}".format(config_file)) 
  
  detector = EfficientDetModelInspector(config_file)
  detector.run_model()
